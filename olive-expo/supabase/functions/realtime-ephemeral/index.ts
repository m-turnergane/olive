...
// @ts-ignore
const REALTIME_MODEL = Deno.env.get('REALTIME_MODEL') ?? 'gpt-4o-mini-realtime-preview-2024-12-17';
// @ts-ignore
const REALTIME_VOICE_DEFAULT = Deno.env.get('REALTIME_VOICE_DEFAULT') ?? 'nova';
// @ts-ignore
const REALTIME_VOICE_FEMALE = Deno.env.get('REALTIME_VOICE_FEMALE') ?? 'nova';
// @ts-ignore
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (REALTIME_ENABLE === 'false') {
      ...
      return new Response(
        JSON.stringify({ ok: false, error: 'Voice features are currently disabled' }),
    ...
    log('debug', `User voice preference: ${voiceGender}, selected voice: ${selectedVoice}`);
    ...
    if (REALTIME_SERVER === 'azure') {
      if (!AZURE_OPENAI_ENDPOINT) {
        log('error', 'AZURE_OPENAI_ENDPOINT required when REALTIME_SERVER=azure');
        return new Response(
    ...
    if (!openaiRes.ok) {
      ...
      log('error', `Sessions API error (${openaiRes.status})`, { error: errorText });
      return new Response(
        JSON.stringify({
          ok: false,
          error: 'Failed to create Realtime session',
          details: LOG_LEVEL === 'debug' ? errorText : undefined,
        }),
